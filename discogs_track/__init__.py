...
@click.pass_context
def artist(ctx, id: click.INT):
    ctx.obj['artist'] = Artist(api=ctx.obj['api'], artist_id=id, verbosity=ctx.obj['verbose'], from_cache=ctx.obj['from_cache'])
...
def show_completing(ctx, for_sale: click.BOOL):
